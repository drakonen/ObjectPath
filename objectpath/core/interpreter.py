    ...
    def execute(self, expr):
        ...
        def exe(node):
            ...
            try:
                ...
                pass
            if D:
                self.start("executing node %s", color.bold(self.cleanOutput(node)))
            type_node = type(node)
            ...
            if op == "or":
                if D:
                    self.debug("%s or %s", node[1], node[2])
                return exe(node[1]) or exe(node[2])
            elif op == "and":
                if D:
                    self.debug("%s and %s", node[1], node[2])
                ...
                if len(node) > 2:
                    ...
                    if typefst in ITER_TYPES or typesnd in ITER_TYPES:
                        if typefst not in ITER_TYPES:
                            ...
                            snd = [snd]
                        if D:
                            self.debug("at least one side is a generator and the other is an iterable, returning chain")
                ...
                if D:
                    self.debug("%s > %s, %s", node[1], node[2], node[1] > node[2])
                return exe(node[1]) > exe(node[2])
            elif op == "<":
                return exe(node[1]) < exe(node[2])
            elif op == ">=":
            ...
            # 		return fst <= snd and snd or False
                ...
                snd = exe(node[2])
                if D:
                    self.debug("doing '%s' in '%s'", node[1], node[2])
                ...
                snd = exe(node[2])
                if D:
                    self.debug("doing '%s' not in '%s'", node[1], node[2])
                if type(fst) in ITER_TYPES and type(snd) in ITER_TYPES:
                    return not any(
                        x in max(fst, snd, key=len) for x in min(fst, snd, key=len)
                    )
                ...
                if typefst in STR_TYPES:
                    ...
                    if D:
                        self.info("can't compare %s and %s. Returning False", self.cleanOutput(fst), self.cleanOutput(snd))
                    ret = False
                # else:
                # 	try:
                # 		global ObjectId
                ...
                if op == "is not":
                    if D:
                        self.info("'is not' found. Returning %s", not ret)
                    return not ret
                else:
                    if D:
                        self.info("returning %s is %s => %s", color.bold(self.cleanOutput(fst)), color.bold(self.cleanOutput(snd)), color.bold(ret))
                    return ret
            elif op == "re":
            ...
            # 	return self.node
                ...
                typefst = type(fst)
                if D:
                    self.debug(color.op(".") + " left is '%s'", color.bold(self.cleanOutput(fst)))
                # try:
                if node[2][0] == "*":
                    if D:
                        self.end(
                ...
                snd = exe(node[2])
                if D:
                    self.debug(color.op(".") + " right is '%s'", color.bold(snd))
                ...
                if type(snd) in ITER_TYPES:
                    ...
                    if D:
                        self.debug(color.op("..") + " returning %s", color.bold(self.cleanOutput(ret)))
                    return ret
            elif op == "[":
                len_node = len(node)
        ...
        ret = exe(tree)
        if D:
            self.end("Tree.execute with: %s", color.bold(self.cleanOutput(ret)))
        return ret
